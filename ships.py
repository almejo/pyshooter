import animation, gutils, constants, weapons
...
class Nave(pygame.sprite.Sprite):
	...
	def __init__(self, center):
		...
		self.x_velocity = 0
		self.y_velocity = 0
		Nave.laser_fx = gutils.load_SoundFile("laser.wav");
		self.key_y_buff = []
		self.key_x_buff = []
	...
	def update(self):
		...
		elif self.rect.right > constants.SCREEN_WIDTH: self.rect.right = constants.SCREEN_WIDTH

		self.image = self.animation.anim()

		if self.rect.top <= 0: self.rect.top = 0
		elif self.rect.bottom >= constants.SCREEN_HEIGHT: self.rect.bottom = constants.SCREEN_HEIGHT

	def proccessKey(self, event, lasers):
		if event.type == KEYDOWN :
			...
			if event.key == K_UP :
				if -4 in self.key_y_buff: self.key_y_buff.remove(-4) 
			if event.key == K_DOWN :
				if 4 in self.key_y_buff: self.key_y_buff.remove(4) 
			if event.key == K_RIGHT:
				if 4 in self.key_x_buff: self.key_x_buff.remove(4)
			elif event.key == K_LEFT:
				if -4 in self.key_x_buff: self.key_x_buff.remove(-4) 

	def get_weapon_bullets(self):
		if self.weapon is not None: return self.weapon.get_bullets()
		return 0
...
class EnemyRing(AEnemy):
	...
	def step(self):
		if self.counter > self.first and self.counter < self.second : self.y_velocity = 0
		elif self.counter > self.second  : self.y_velocity = 4
