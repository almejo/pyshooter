...
def main():
    ...
    bomb = gutils.load_sound_file("bomb.wav")
    ship_explosion = gutils.load_sound_file("ship_explosion.wav")
    caching = gutils.load_sound_file("chaching.wav")
    ...
    while running is True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_ESCAPE:
                    ...
                    ship.process_key(event, lasers)
            elif event.type == KEYUP:
                ship.process_key(event, lasers)

        ship_sprite.clear(screen, background_image)
        ...
        for hit in pygame.sprite.groupcollide(enemies, lasers, 1, 1):
            explosions.add(animation.Explosion(hit.rect.center))
            bomb.play()
            score += 1
            if score % 10 == 0: caching.play()

        for hit in pygame.sprite.groupcollide(ship_sprite, enemy_shoots, 1, 1):
            explosions.add(animation.Explosion(hit.rect.center))
            ship = ships.Nave((constants.SCREEN_WIDTH / 2, constants.SCREEN_HEIGHT - 1))
            ship_sprite.add(ship)
            ship_explosion.play()
        ...
        for hit in pygame.sprite.groupcollide(ship_sprite, enemies, 1, 0):
            explosions.add(animation.Explosion(hit.rect.center))
            ship = ships.Nave((constants.SCREEN_WIDTH / 2, constants.SCREEN_HEIGHT - 1))
            ship_sprite.add(ship)
            ship_explosion.play()
